...
struct Extend<H: MacMarker> {
    // hmac
    mac: Box<Hmac<H>>,
    ik_c_s: Vec<u8>,
    ik_s_c: Vec<u8>,
}
impl<H: MacMarker> Extend<H> {
    fn from(mac: Box<Hmac<H>>, ik_c_s: Vec<u8>, ik_s_c: Vec<u8>) -> Self {
        Extend {
            mac,
            ik_c_s,
            ik_s_c,
