    ...
    S: Read + Write,
{
    ...
    pub fn upload<P: AsRef<OsStr> + ?Sized>(
        ...
        tracing::info!("files upload successful.");
    ...
    fn file_all(&mut self, scp_file: &mut ScpFile) -> SshResult<()> {
        ...
        if scp_file.local_path.is_dir() {
            ...
            if let Err(e) = fs::read_dir(scp_file.local_path.as_path()) {
                tracing::error!("read dir error, error info: {}", e);
                return Ok(());
            }
            ...
            for p in fs::read_dir(scp_file.local_path.as_path()).unwrap() {
                match p {
                    ...
                    Err(e) => {
                        // 暂不处理
                        tracing::error!("dir entry error, error info: {}", e);
                    }
                }
            }
    ...
    fn send_file(&mut self, scp_file: &mut ScpFile) -> SshResult<()> {
        ...
        tracing::debug!("file: [{}] upload completed.", scp_file.name);
    ...
    fn send_dir(&mut self, scp_file: &ScpFile) -> SshResult<()> {
        ...
        tracing::debug!("dir: [{}] upload completed.", scp_file.name);
    ...
    S: Read + Write,
{
    ...
    pub fn download<P: AsRef<OsStr> + ?Sized>(
        ...
        tracing::info!("files download successful.");
    ...
    fn process_dir_d(&mut self, data: Vec<u8>, scp_file: &mut ScpFile) -> SshResult<()> {
        ...
        tracing::debug!("dir: [{}] download completed.", scp_file.name);
        Ok(())
    }
    ...
    fn save_file(&mut self, scp_file: &mut ScpFile) -> SshResult<()> {
            ...
            .open(path.as_path())
        {
            Ok(v) => v,
            Err(e) => {
                tracing::error!("file processing error, error info: {}", e);
        ...
        tracing::debug!("file: [{}] download completed.", scp_file.name);
        Ok(())
    }
