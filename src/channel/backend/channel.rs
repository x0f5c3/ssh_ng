...
impl Channel {
        ...
        S: Read + Write,
    {
        if !self.is_close() {
            ...
            Err(CrateError::from(SshError::from("Send data on a closed channel")))
        }
    }
    ...
    pub fn local_close(&mut self) -> SshResult<()> {
        tracing::trace!("Channel {} send local close", self.client_channel_no);
        self.local_close = true;
        Ok(())
    }

    pub fn remote_close(&mut self) -> SshResult<()> {
        tracing::trace!("Channel {} recv remote close", self.client_channel_no);
...
impl Drop for Channel {
    fn drop(&mut self) {
        tracing::info!("Channel {} closed", self.client_channel_no);
    }
}
...
impl ChannelBroker {
    ...
    pub(super) fn send(&self, data: Data) -> SshResult<()> {
        ...
        if !self.close {
            match self.rcv.recv().unwrap() {
                BackendResp::Ok(_) => {
                    tracing::trace!("{}: control command ok", self.client_channel_no)
                }
                BackendResp::Fail(msg) => tracing::error!(
    ...
    pub(super) fn try_recv(&mut self) -> SshResult<Option<Vec<u8>>> {
        if !self.close {
            ...
            Err(CrateError::from(SshError::from("Read data on a closed channel")))
        }
    }
