...
pub(crate) fn from_utf8(v: Vec<u8>) -> SshResult<String> {
    match String::from_utf8(v) {
        ...
        Err(e) => {
            ...
            Err(CrateError::from(SshError::from(err_msg)))
        }
    }
}
...
pub(crate) fn sys_time_to_secs(time: SystemTime) -> SshResult<u64> {
    match time.duration_since(UNIX_EPOCH) {
        Ok(t) => Ok(t.as_secs()),
        Err(e) => Err(CrateError::from(SshError::from(format!(
...
pub(crate) fn str_to_i64(v: &str) -> SshResult<i64> {
    match i64::from_str(v) {
        Ok(v) => Ok(v),
        Err(_) => Err(CrateError::from(SshError::from("str to i64 error"))),
    }
}

pub(crate) fn check_path(path: &Path) -> SshResult<()> {
    if path.to_str().is_none() {
        return Err(CrateError::from(SshError::from("invalid path.")));
    }
    Ok(())
}
