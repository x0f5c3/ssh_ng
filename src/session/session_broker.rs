        ...
        S: Read + Write + Send + 'static,
    {
        ...
        spawn(move || {
            if let Err(e) = client_loop(client, stream, rqst_rcv) {
                tracing::error!("Error {} occurred when running backend task", e.to_string())
            }
        });
        Self {
        ...
        if try_recv.is_err() {
            ...
            match rqst {
                BackendRqst::OpenChannel(id, data, sender) => {
                    tracing::info!("try open channel {}.", id);
            ...
            match message_code {
                ...
                */
                ...
                ssh_msg_code::SSH_MSG_CHANNEL_DATA => {
                    let id = data.get_u32();
                    tracing::trace!("Channel {id} get {} data", data.len());
                    let channel = channels.get_mut(&id).unwrap();
                ...
                ssh_msg_code::SSH_MSG_CHANNEL_CLOSE => {
                    let id = data.get_u32();
                    tracing::info!("Channel {} recv close", id);
                    let channel = channels.get_mut(&id).unwrap();
                ...
                x @ ssh_msg_code::SSH_MSG_CHANNEL_EOF => {
                    tracing::debug!("Currently ignore message {}", x);
                }
                ...
                _x @ ssh_msg_code::SSH_MSG_CHANNEL_SUCCESS => {
                    let id = data.get_u32();
                    tracing::trace!("Channel {} control success", id);
                    let channel = channels.get_mut(&id).unwrap();
                    channel.success()?
                }
                ssh_msg_code::SSH_MSG_CHANNEL_FAILURE => {
                    let id = data.get_u32();
                    tracing::trace!("Channel {} control failed", id);
                    let channel = channels.get_mut(&id).unwrap();
                    channel.failed()?
                }
