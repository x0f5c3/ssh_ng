...
impl Client {
        ...
        loop {
            ...
            match message_code {
                ...
                ssh_msg_code::SSH_MSG_USERAUTH_FAILURE => {
                    if !tried_public_key {
                        tracing::error!("user auth failure. (public key)");
                        tracing::info!("fallback to password authentication");
                        tried_public_key = true;
                        ...
                        tracing::error!("user auth failure. (password)");
                        return Err(SshError::from("user auth failure."));
                    }
                }
                ssh_msg_code::SSH_MSG_USERAUTH_PK_OK => {
                    tracing::info!("user auth support this algorithm.");
    ...
    where
        S: Write,
    {
        tracing::info!("password authentication.");
        let mut data = Data::new();
